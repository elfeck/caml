...
def singleBasis3():
    ...
    t3 = plt.text(point + 0.12, alpha - 0.6, r'$\phi_3(x)$', color="blue")
    t3.set_fontsize(16)
...
def singleBasis4():
    ...
    plt.plot(xval, ys_a, c="blue")
    plt.plot(xval, ys, c="grey", linestyle="dotted")

    # alpha line
    plt.plot((point, point), (0, alpha), c="blue", linestyle="dashed")

    # labels
    t1 = plt.text(point + 0.01, alpha - 0.6, r'$\alpha_3$', color="blue")
    t1.set_fontsize(12)

    t2 = plt.text(point + 0.12, alpha - 0.6,
                  r'$\alpha_3 \cdot \phi_3(x)$', color="blue")
    t2.set_fontsize(16)

    t3 = plt.text(point + 0.117, alpha -0.5, r'$\phi_3(x)$', color="grey")
    t3.set_fontsize(12)
...
def singleBasis5():
    ...
    for g in gridpoints:
        ...
        plt.plot(xval, ys_a[-1], c="blue", linestyle="dotted")
    ...
    for ys_i in ys_a:
        for i in range(len(ys_i)):
            ys_sum[i] += ys_i[i]
    plt.plot(xval, ys_sum, c="blue", linewidth=2)
